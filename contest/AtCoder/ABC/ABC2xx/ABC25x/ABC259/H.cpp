...
int main(){
    ...
    rep(i,0,n) rep(j,0,n){
        cin >> a[i][j]; a[i][j]--;
        cnt[a[i][j]].emplace_back(i,j);
    }
    mint ans = 0;
    rep(k,0,n*n){
        if(cnt[k].size()<=n/2){
            rep(i,0,cnt[k].size()) rep(j,0,cnt[k].size()){
                if(cnt[k][i].first <= cnt[k][j].first and cnt[k][i].second <= cnt[k][j].second){
                    ans += f(cnt[k][j].first-cnt[k][i].first,cnt[k][j].second-cnt[k][i].second);
                }
            }
        }
        else{
            ...
            rep(i,0,n) rep(j,0,n) if(a[i][j]==k) ans += dp[i][j];
        }
    }
    cout << ans.val() << endl;
}