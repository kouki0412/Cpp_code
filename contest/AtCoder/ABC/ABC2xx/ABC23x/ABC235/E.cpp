...
int main(){
    ...
    dsu D(n);
    while(!que.empty()){
        ll x,index,f; tie(x,index,f)=que.top(); que.pop();
        if(f){
            ll a,b; tie(a,b)=qab[index];
            if(!D.same(a,b)) ans[index]=1;
        }
        else{
            ll a,b; tie(a,b)=ab[index];
